...
module.exports.run = (client, message, args, Prefix) => {
    let invitation = "[``Ajouter le bot``](https://discord.com/api/oauth2/authorize?client_id=771744027140947969&permissions=838768&scope=bot)"
    ...
    if(!args.length) {
        ...
        for(let category of categoryList) {
            ...
            if(category[0] != 0) {
                if(client.commands.filter(cat => cat.help.category.toLowerCase() == category[1].toLowerCase()).map(cmd => `__${cmd.help.name}__`).join(', ')) {
                    ...
                    E.addField(`${catName}`, `${client.commands.filter(cat => cat.help.category.toLowerCase() == category[1].toLowerCase()).map(cmd => `__${cmd.help.name}__`).join(', ')}`, true)
                }
            }
        }
        E.addField("Lien important", `${invitation}`)
    } else {
        ...
        .addField(`Cooldown`, `**${command.help.cooldown}** secondes.`, true)
